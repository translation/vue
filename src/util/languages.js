...
export const fetchSegmentsFromLanguageFiles = (config) => {
    ...
    const languageFilesPath = resolve(process.cwd(), config.translations_directory)

    const languageFiles = readdirSync(languageFilesPath)
        .map((file) => {
            const languagePath = resolve(process.cwd(), config.translations_directory, file)
    ...
    return languageFiles.reduce((accumulator, file) => {
        ...
        const segmentsInFile = Object.keys(flattenedObject).map((key, index) => {

...
export default function(config) {
    ...
    if (segmentsFromVueFiles.length === 0) {
        log.error(
            'No translations found in your vue files. Please check your `source_path` config and make sure you have localized strings in your code.'
        )
    ...
    if (Object.keys(segmentsFromLanguageFiles).length === 0) {
        log.error(
            'No language files where found. Please check your `translations_directory` config.'
        )
    ...
    const sourceTranslations =
        segmentsFromLanguageFiles[config.source_locale] || []

    const sourceSegments = segmentsFromVueFiles.map(segment => {
        if (config.translations_type === 'key') {
            const sourceTranslation = sourceTranslations.find(
                source => source.key === segment.path
            )
    ...
    if (config.translations_type === 'key') {
        ...
        sourceTranslations.forEach((translation, index) => {
            if (
                sourceSegments.findIndex(
                    segment => segment.key === translation.key
                ) < 0
            ) {
    ...
    if (config.translations_type === 'key') {
        api.pull().then(data => {
            ...
            data.source_edits.forEach(edit => {
                const translationIndex = sourceTranslations.findIndex(
                    translation => {
                ...
                if (translationIndex >= 0) {
                    log.info(
                        `Replacing old traduction \`${edit.new_source}\` by \`${edit.old_source}\` for key \`${edit.key}\`.`
                    )

                    sourceTranslations[translationIndex].target =
                        edit.new_source
                }
            })
    ...
    api.sync(sourceSegments, config.readonly, config.purge).then(
        data => {
            Object.keys(data.segments).forEach(locale => {
                const translations = {}
            ...
            if (config.purge) {
                // Perform purge
                log.success(
                    `\`${data.unused_segment_ids.length}\` segments removed.`
                )
            }

            log.success(
                `Project \`${data.project.name}\` synchronized with success.`
            )
        },
        error => {

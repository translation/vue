...
export default function(config) {
    ...
    if (config === null) {
        log.error(
            'The configuration file is missing. Please create a `translation.json` configuration file at the root of your project.'
        )
    ...
    segmentsFromVueFiles.forEach(segment => {
    ...
    Object.keys(segments).forEach(locale => {
        ...
        if (languageTranslations) {
            languageTranslations.forEach(translation => {
                ...
                if (config.translations_type === 'key') {
                    const segmentIndex = segments[locale].findIndex(
                        segment => segment.key === translation.key
                    )
                    if (segmentIndex >= 0) {
                        segments[locale][segmentIndex].target =
                            translation.target
                    }
                } else {
                    const segmentIndex = segments[locale].findIndex(
                        segment => segment.source === translation.source
                    )
                    if (segmentIndex >= 0) {
                        segments[locale][segmentIndex].target =
                            translation.target
    ...
    translation.init(segments).then(
        data => {
            log.success(
                `Project \`${data.project.name}\` initialized with success.`
            )
